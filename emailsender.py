...
def send_email(full_name, email, drink_choice):
    ...
    s.starttls()
    s.login(fromaddr, "dwdm zijj jvss lfmv")
    text = msg.as_string()
...
def actions(full_name, email, drink_choice):
...
if __name__ == '__main__':
    #import csv from tobesent.csv
    with open('tobesent.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
            email = row[1]
            full_name = row[0]
            drink_choice = row[2]
            actions(full_name, email, drink_choice)
